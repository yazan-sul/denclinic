...
export default function StaffDashboard() {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      setError(null);
      try {
        const params = new URLSearchParams({ from: today, to: today, activeRole: 'STAFF' });
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {/* Welcome */}
      <div className="bg-gradient-to-l from-primary/5 to-primary/10 rounded-2xl p-4 md:p-6 border border-primary/20">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <div className="bg-card border border-border rounded-xl p-3 md:p-4">
          ...
          <p className="text-xs md:text-sm text-muted-foreground mt-1">مواعيد اليوم</p>
        </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 p-3 md:p-4 border-b border-border">
          <div className="flex items-center gap-2 md:gap-3 flex-wrap">
            <h3 className="text-base md:text-lg font-bold text-foreground">مواعيد اليوم</h3>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            <div className="relative flex-1 md:flex-none">
              <SearchIcon className="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground flex-shrink-0" />
              <input
                type="text"
                placeholder="ابحث..."
                value={search}
        ...
        {/* Table */}
        {loading ? (
          <div className="text-center py-8 md:py-12 text-muted-foreground text-sm md:text-base">جاري التحميل...</div>
        ) : error ? (
          <div className="text-center py-8 md:py-12 text-destructive text-sm md:text-base">{error}</div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-8 md:py-12 text-muted-foreground text-sm md:text-base">لا توجد مواعيد</div>
